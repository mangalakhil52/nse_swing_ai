...
│ * NOTE: Yahoo Finance is explicitly excluded due to data quality issues.│
└─────────────────────────────────────────────────────────────────────────┘
```
...
All data access is decoupled via Python Abstract Base Classes (`ABC`). Application components depend exclusively on abstract contracts, making data vendors hot-swappable with zero core logic changes.
...
class MarketDataProvider(ABC):
    ...
    async def get_historical_ohlcv(
        ...
        ['open', 'high', 'low', 'close', 'volume', 'delivery_volume', 'turnover']"""
        pass
...
### B. Fundamental Provider (`FundamentalProvider`)
```python
...
### C. News & Catalyst Provider (`NewsProvider`)
```python
class NewsProvider(ABC):
    ...
    async def fetch_news_feed(
        ...
        """Returns Tier 1/2 news articles with URL, source, author, timestamp, text.""                          Try Primary Provider (NSE Official Bhavcopy/API)
                                        │
                        ┌───────────────┴───────────────┐
                        │ Success?                      │
                        ├─────────────────┬─────────────┤
...
| **Corporate Announcements** | BSE / NSE Corporate Feed API | Moneycontrol Verified Desks | 4x daily | Extracted timestamp $\le 24$h old |
...
| Data Category | Primary Source | Secondary Fallback | Refresh Cadence | Validation Rule |
|---|---|---|---|---|
| **Daily EOD OHLCV + Delivery** | NSE Bhavcopy & Sec_Bhav | Yahoo Finance (`symbol.NS`) | Daily at 15:45 IST | Zero volume reject, split continuity |
| **Market Indices (Nifty 50, Sectorals)** | NSE Index Bhavcopy | Yahoo Finance (`^NSEI`, `^CNXIT`) | Daily at 15:45 IST | Close within High-Low range |
| **Corporate Announcements** | BSE / NSE Corporate Feed API | Google News RSS / Moneycontrol | 4x daily | Extracted timestamp $\le 24$h old |
| **Earnings Calendar** | NSE Board Meetings Feed | Trendlyne / Screener | Daily at 08:30 IST | Exact date verification |
| **Fundamentals & Ratios** | Screener.in API / Financial RSS | Yahoo Finance Fundamentals | Weekly | CFO vs PAT sign & magnitude check |
| **Surveillance Lists (ASM/GSM)** | NSE Official Circulars | BSE Notice Feed | Daily at 08:00 IST | Strict binary flag (In/Out) |
