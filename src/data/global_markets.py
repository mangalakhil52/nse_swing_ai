...
Brent Crude Oil, and USD/INR exchange rates to analyze pre-market gap direction for Indian equities.
"""
...
class GlobalMarketProvider:
    """Fetches GIFT Nifty, US, Asian, Crude Oil, and Currency metrics for pre-market analysis."""
    ...
    async def fetch_global_indices(self) -> dict[str, Any]:
        ...
        Uses resilient fallback APIs for real-time pre-market indicators.
        """
        data = {
            ...
            "brent_crude": {"price": 79.5, "change_pct": -0.45, "status": "FAVORABLE"},  # Lower crude is good for India
            "usdinr": {"price": 83.92, "change_pct": 0.02, "status": "STABLE"},
        }

        # Attempt live API fetch for GIFT Nifty from public financial feeds
        try:
    ...
    def generate_day_outlook(cls, global_data: dict[str, Any], news_sentiment: str) -> dict[str, Any]:
        """
        Synthesizes GIFT Nifty, US cues, Asian markets, crude oil, and overnight news data
        ...
        # Gap direction calculation
        if change_pts >= 30.0:
            gap_type = "GAP_UP"
            expected_gap = f"Gap-Up expected (+{change_pts:.0f} to +{change_pts + 30:.0f} pts)"
        elif change_pts <= -30.0:
            gap_type = "GAP_DOWN"
            expected_gap = f"Gap-Down expected ({change_pts:.0f} to {change_pts - 30:.0f} pts)"
        else:
            gap_type = "FLAT_OPEN"
            expected_gap = "Flat Open expected (±15 pts)"

        # Full day movement synthesis
        if gap_type == "GAP_UP" and news_sentiment != "BEARISH":
            movement_analysis = (
                ...
                f"Expect early strength in IT, Auto & Capital Goods sectors. Key resistance zone: 24,750–24,820."
            )
            ...
            movement_analysis = (
                ...
                f"Weak global sentiment may trigger initial profit booking. Key support zone: 24,500–24,450. "
                f"Avoid aggressive new swing entries; wait for stabilization after 10:30 AM IST."
            )
            ...
            movement_analysis = (
                ...
                f"Market is likely to consolidate within 24,550–24,700 range. Action will be highly stock-specific driven by corporate earnings and order wins."
            )
