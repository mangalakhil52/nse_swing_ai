...
class CandidateDiscoveryConfig(BaseModel):
    ...
    require_trend_alignment: bool = Field(
        default=False,
        description="Optional basic screen only; not specialist technical analysis.",
    )
...
class CandidateDiscoveryResult(BaseModel):
    ...
    discovery_score: float | None = Field(
        default=None,
        description="Screening metric only (ADTV in INR crore); not conviction.",
    )
...
class CandidateDiscoveryEngine:
    ...
    def _resolve_mode(
        cls,
        universe: Sequence[str] | Sequence[SymbolMetadata],
        mode: str | None,
    ) -> str:
        if mode is not None:
    ...
    def _resolve_universe(
        ...
        if mode == "HISTORICAL":
            ...
            return HistoricalUniverseProvider.get_universe_for_date(
                as_of_date.date() if isinstance(as_of_date, datetime) else as_of_date,
                securities=metadata,
            )
    ...
    def discover_candidates(
        ...
        if universe is None:
            ...
            if resolved_mode.upper() not in {"HISTORICAL", "LIVE"}:
                raise ValueError("mode must be 'HISTORICAL' or 'LIVE'")
            resolved_universe = cls._resolve_universe(None, as_of_date, resolved_mode.upper())
        else:
        ...
        for symbol in resolved_universe:
            ...
            if sliced_df is None or sliced_df.empty:
                results.append(
                    cls._empty_result(symbol, as_of_date, "NO_DATA_AVAILABLE_BEFORE_AS_OF_DATE", pit_safe=True)
                )
                continue
            ...
            window = (
                sliced_df.sort_values("timestamp").tail(cfg.liquidity_window)
                if "timestamp" in sliced_df.columns
                else sliced_df.tail(cfg.liquidity_window)
            )
