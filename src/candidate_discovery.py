"""Deterministic Stage-1 candidate discovery.

Candidate discovery is intentionally upstream of all specialist agents. It
accepts an already-selected universe, applies the existing PIT/data-quality
contracts, and performs only cheap, explainable market-data screening.
"""
...
class CandidateDiscoveryConfig(BaseModel):
    """Explicit Stage-1 screening configuration; no agent/conviction scoring."""
    ...
    require_trend_alignment: bool = Field(default=False, description="Optional basic screen only; not specialist technical analysis.")


class CandidateDiscoveryResult(BaseModel):
    """Explainable result for one symbol at one decision timestamp."""
    ...
    pit_safe: bool = False
    discovery_score: float | None = Field(default=None, description="Screening metric only (ADTV in INR crore); not conviction.")
...
class CandidateDiscoveryEngine:
    ...
    def _resolve_universe(
        cls,
        universe: Sequence[str] | Sequence[SymbolMetadata] | None,
        ...
        if universe is None:
            if mode == "HISTORICAL":
                # Never silently substitute the current universe for a historical date.
                # Historical scans must provide survivorship-safe SymbolMetadata.
                raise TypeError(
                    "Historical candidate discovery requires explicit SymbolMetadata with listing/delisting metadata"
                )
        ...
        if mode == "HISTORICAL":
            ...
            if not metadata or not all(isinstance(item, SymbolMetadata) for item in metadata):
                raise TypeError(
                    "Historical candidate discovery requires SymbolMetadata with listing/delisting metadata"
                )
            return HistoricalUniverseProvider.get_universe_for_date(as_of, securities=metadata)

        return [
            item.symbol.strip().upper() if isinstance(item, SymbolMetadata) else str(item).strip().upper()
    ...
    def discover_candidates(
        cls,
        universe: Sequence[SymbolMetadata] | Sequence[str] | None,
        as_of_date: datetime | date,
        market_data_map: dict[str, pd.DataFrame],
        config: CandidateDiscoveryConfig | None = None,
        mode: str | None = None,
    ) -> list[CandidateDiscoveryResult]:
        """Screen an input universe without making any trade/conviction decision."""
        cfg = config or CandidateDiscoveryConfig()
        if universe is None:
            resolved_mode = (mode or "LIVE").upper()
            if resolved_mode not in {"HISTORICAL", "LIVE"}:
                raise ValueError("mode must be 'HISTORICAL' or 'LIVE'")
        else:
            resolved_mode = cls._resolve_mode(universe, mode)
        ...
        for symbol in resolved_universe:
            ...
            raw_df = market_data_map.get(symbol) or market_data_map.get(symbol.lower())
            ...
            if "turnover_crores" in window.columns:
                adtv_crores = float(window["turnover_crores"].astype(float).mean())
            else:
                adtv_crores = float((window["close"].astype(float) * window["volume"].astype(float)).mean() / 1e7)

            volume_ok = avg_volume >= cfg.min_average_volume
            turnover_ok = adtv_crores >= cfg.min_average_turnover_crores
            liquidity_ok = volume_ok and turnover_ok
