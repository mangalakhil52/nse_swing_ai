...
class NewsIntelligenceAgent(BaseAgent):
    ...
    async def _analyze(self, symbol_meta: SymbolMetadata, df: pd.DataFrame, evidence_graph: EvidenceGraph, run_id: str, context: dict[str, Any]) -> AgentOutput:
        ...
        if as_of is not None:
            ...
            articles, announcements = raw_articles, raw_announcements
        if not articles and not announcements:
            return AgentOutput(agent_name=self.agent_name, symbol=symbol, run_id=run_id, status=AgentStatus.DATA_UNAVAILABLE,
                signal=SignalType.NEUTRAL, score=0.0, confidence=None, data_freshness=DataFreshness.UNKNOWN,
                metrics={"articles_found": 0, "announcements_found": 0, "status": "NEWS_UNAVAILABLE_OR_PIT_UNVERIFIED"},
                risks_identified=["No point-in-time verified news/event evidence available."], evidence=[])
        ...
        score = 65.0
        if surprise_pct > 10.0: score += 15.0
        elif surprise_pct > 0.0: score += 8.0
        elif surprise_pct < -10.0: score -= 25.0
        if unpriced_catalyst_count > 0: score += min(15.0, unpriced_catalyst_count * 7.5 * avg_materiality)
        if already_priced > 0: score -= min(10.0, already_priced * 4.0)
        if pos_count > neg_count: score += 10.0; signal = SignalType.BULLISH
        elif neg_count > pos_count: score -= 20.0; signal = SignalType.BEARISH
        else: signal = SignalType.NEUTRAL
        score = min(100.0, max(0.0, score))
        for article in articles[:3]:
            pub = getattr(article, "available_at", None) or getattr(article, "published_at", None)
            if pub is not None:
                evidence_graph.add_evidence(symbol=symbol, agent_name=self.agent_name, claim_type="VERIFIED_NEWS_MATERIALITY",
                    raw_metric="materiality_score", observed_value=f"[{article.publisher}] {article.headline} (Materiality: {article.materiality_score:.2f})",
                    unit="materiality_index", source=article.publisher, timestamp=pub.isoformat() if hasattr(pub, "isoformat") else str(pub), citation_url=article.source_url)
        risks = []
        if neg_count > 0: risks.append(f"Identified {neg_count} negative press/filing headlines in the verified window.")
        if surprise_pct < -5.0: risks.append(f"Earnings miss detected: {abs(surprise_pct):.1f}% below reference consensus.")
        ...
        return AgentOutput(agent_name=self.agent_name, symbol=symbol, run_id=run_id, status=AgentStatus.SUCCESS,
            signal=signal, score=round(score, 1), confidence=confidence, data_freshness=DataFreshness.RECENT,
            metrics={"positive_articles": pos_count, "negative_articles": neg_count, "total_announcements": len(announcements),
                     "avg_materiality": avg_materiality, "earnings_surprise_pct": surprise_pct, "unpriced_catalyst_count": unpriced_catalyst_count},
            evidence=evidence_graph.to_evidence_items(symbol), risks_identified=risks)

    async def analyze_contract(self, symbol_meta: SymbolMetadata, df: pd.DataFrame, decision_time: datetime | date,
                               run_id: str = "", context: dict[str, Any] | None = None) -> AgentAnalysisResult:
        ...
        structured = [StructuredEvidence(source="NEWS", observation=item.observed_value, as_of=decision_time, direction=signal,
            strength="HIGH" if output.score >= 75 else ("MEDIUM" if output.score >= 55 else "LOW"),
            reliability=1.0 if pit_safe else 0.0, pit_safe=pit_safe)
            for item in graph.to_evidence_items(symbol_meta.symbol)]
        return AgentAnalysisResult(symbol=symbol_meta.symbol.upper().strip(), agent_name=self.agent_name, decision_time=decision_time,
            signal=signal, score=output.score if pit_safe else 0.0,
            confidence=output.confidence if pit_safe and output.confidence is not None else 0.0, evidence=structured,
            risks=output.risks_identified, catalysts=["UNPRICED_NEWS_CATALYST"] if pit_safe and output.metrics.get("unpriced_catalyst_count", 0) else [],
            data_quality=None, pit_safe=pit_safe, status=output.status if pit_safe else AgentStatus.DATA_UNAVAILABLE,
