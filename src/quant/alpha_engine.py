...
class CrossSectionalAlphaEngine:
    ...
    @classmethod
    def rank(cls, stock_dfs: dict[str, pd.DataFrame], benchmark_df: pd.DataFrame, regime: MarketRegime) -> list[AlphaSnapshot]:
        features = {}
        ...
        for symbol, f in features.items():
            ...
            snapshots.append(AlphaSnapshot(symbol, round(float(score), 6), {
                "momentum_21": f.momentum_21, "momentum_63": f.momentum_63, "momentum_126": f.momentum_126,
                "trend_quality": f.trend_quality, "volatility": f.volatility, "downside_volatility": f.downside_volatility,
                "volume_surprise": f.volume_surprise, "range_compression": f.range_compression,
                "breakout_distance": f.breakout_distance, "relative_strength": f.relative_strength,
            }, regime))
