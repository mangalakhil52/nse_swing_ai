...
async def execute_daily_5pm_cycle(target_date: date | None = None, force: bool = False) -> int:
    ...
    if not force and not MarketCalendar.is_trading_day(target_date):
        logger.info("Non-trading day: %s", target_date)
        return 0
    ...
    indianapi: IndianAPIProvider | None = None
    ...
    try:
        ...
        async def load_one(meta: SymbolMetadata):
            async with semaphore:
                try:
                    provider = upstox if upstox is not None else hist_provider
        ...
        # Complete market-regime inputs. Upstox is optional for the UI, but it is
        # required for live signal mode because the regime must have genuine NIFTY
        # history; no equity-symbol substitute is accepted.
        if upstox is None:
            raise DataUnavailableException("SIGNAL_MODE_REQUIRES_UPSTOX: configure UPSTOX_ACCESS_TOKEN for live NIFTY regime data")
        nifty_df = await upstox.get_index_daily_ohlcv("NIFTY 50", start_history_date, target_date)
        ...
        # The empirical store is a hard prerequisite for live signals.
        ...
        logger.info("Technical candidates: %d", len(candidates))
        ...
        recommendations = await cio.run_daily_scan(
            ...
            stock_dfs=stock_dfs,
            universe={m.symbol: m for m in universe_meta},
